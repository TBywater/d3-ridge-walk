<script>
  ...
  const x = d3.scaleLinear().domain([-200, 1900]).range([0, width]);

  const y = d3.scaleLinear().range([height, 0]).domain([0, 0.0035]);
  // Compute kernel density estimation
  const kde = kernelDensityEstimator(kernelEpanechnikov(170), x.ticks(500));
  $: density1 = kde(obj);
  $: density2 = kde(objb);
